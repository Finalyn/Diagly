...
import { mockProjects, mockDiagnostics, statusLabels, statusColors, buildingTypeLabels } from '@/data/mock'
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs md:text-sm text-muted-foreground mt-1">
            <span className="flex items-center gap-1"><MapPin className="h-3.5 w-3.5" />{project.address}, {project.postalCode ? `${project.postalCode} ` : ''}{project.city} ({project.canton})</span>
            {project.yearBuilt && <span className="flex items-center gap-1"><Calendar className="h-3.5 w-3.5" />{project.yearBuilt}</span>}
            <span className="flex items-center gap-1"><Building2 className="h-3.5 w-3.5" />{buildingTypeLabels[project.buildingType]}</span>
          </div>
        </div>
